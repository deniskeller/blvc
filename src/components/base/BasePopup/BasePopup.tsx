...
const BasePopup: React.FC<Props> = ({
	...
	return (
		<BasePortal>
			<div className={`${styles.Wrapper} ${animationIn ? styles.Visible : ''}`}>
				<div className={styles.Overlay} onClick={() => onClick(false)}></div>

				<div
					className={`${styles.Popup} ${size ? styles['Popup_' + size] : ''} ${
						type ? styles['Popup_' + type] : ''
					}`}
				>
					{type != 'confirm' ? (
						<div className={styles.Popup_Close} onClick={() => onClick(false)}>
							<svg
